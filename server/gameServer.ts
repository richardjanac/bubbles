...
export class GameServer {
  ...
  private clientUpdateRates: Map<string, number> = new Map(); // Adaptívny update rate per klient
  ...
  private setupSocketHandlers() {
    this.io.on('connection', (socket) => {
      ...
      socket.on('updateInput', (input: PlayerInput) => {
        ...
        if (input.sequence) {
          (player as any).lastProcessedInput = input.sequence;
        }
  ...
  private startGameLoop() {
    ...
    this.updateInterval = setInterval(() => {
      ...
      for (const [socketId, socket] of this.io.sockets.sockets) {
        ...
        if (Math.floor(currentTime / (1000 / GAME_SETTINGS.GAME_LOOP_FPS)) % updateRate !== 0) {
          continue;
        }
